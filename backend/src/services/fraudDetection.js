 ...
 *
 * Daily limit     : blocks if the UTC-calendar-day total sent so far plus the
 *                   new amount would exceed FRAUD_DAILY_LIMIT_USD.
 *
...
/** Rolling window in hours for velocity check (default 24 h) */
...
/** Max USD-equivalent total sent within the calendar day (default 1000) */
...
const FRAUD_RULES = {
  VELOCITY_TRANSACTIONS: {
    ...
    window: parseInt(process.env.FRAUD_VELOCITY_WINDOW || '10'),
  },
  LARGE_TRANSACTION: {
  ...
  UNIQUE_RECIPIENTS: {
    ...
    window: parseInt(process.env.FRAUD_UNIQUE_RECIPIENTS_WINDOW || '60'),
  },
  DAILY_LIMIT: {
...
/**
 * Daily limit check — returns true (exceeded) when the UTC calendar-day total
 * already sent plus the new amount would exceed FRAUD_DAILY_LIMIT_USD.
 *
 * Uses DATE_TRUNC('day', NOW() AT TIME ZONE 'UTC') so the window always resets
 * at UTC midnight regardless of the server's local timezone.
 *
 * @param {string} walletAddress
